...
pub fn read_terminal_command(input: Option<&Vec<Value>>) -> OpenTerminalCommand {
    let command = if let Some(vals) = input {
        let cmds: Vec<_> = vals.iter().map(|v| v.as_str().expect("malformed terminal arg string").to_string()).collect();
        let cmd = cmds.first().expect("Command to open terminal is misformed. please provide at least one command");
        OpenTerminalCommand {
