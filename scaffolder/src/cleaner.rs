...
fn delete_dirs(dirs_to_delete: Vec<PathBuf>) {
    for dir in dirs_to_delete {
        match fs::remove_dir_all(&dir) {
            Ok(_) => {
                log::info!("Deleted {}", dir.to_string_lossy())
            }
            Err(e) => {
            ...
            .filter_map(|entry| {
                entry.ok().and_then(|e| {
                    ...
                    let deletable = fs::read_dir(&path).map(|proj_entries| {
                        let entries = proj_entries
                            .filter(|pentry| match pentry {
                                Ok(pe) => {
                                    if pe.file_name() == "out" {
                                        return check_for_empty_out(pe.path());
                                    }
                                    let hidden_file =
                                        pe.file_name().to_string_lossy().starts_with(".");
                    ...
                    return deletable
                        .ok()
                        .and_then(|d| if !d { Some(path) } else { None });
                })
            })
            .collect::<Vec<PathBuf>>();
    }
